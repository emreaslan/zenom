...
class Array 
{
	...
	public:
		...
		const T* data() { return d_array; };

		int size() { return d_numElements; }
...
{
	if(index > -1 && index < d_numElements)
		return (d_array[index]);
	else
		throw std::out_of_range("Array length is out of range.\n");
}
