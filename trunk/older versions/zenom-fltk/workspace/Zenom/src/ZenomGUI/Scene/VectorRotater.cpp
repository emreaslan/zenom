...
VectorRotater::VectorRotater(const Vector3D &pLineBegin, const Vector3D& pLineDir, const double pDegree)
{
	setRotationValues(pLineBegin, pLineDir, pDegree);
}

void VectorRotater::setRotationValues(const Vector3D &pLineBegin, const Vector3D& pLineDir, const double pDegree)
{
...
Vector3D VectorRotater::rotate(const Vector3D &pVec)
{
	if (!isInitialized)
	{
