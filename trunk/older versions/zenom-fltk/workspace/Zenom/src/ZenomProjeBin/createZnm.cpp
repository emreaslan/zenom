...
int CreateZnm::init( char *name ){
	...
	oXmlFile = new char[len + 2];
	snprintf( oXmlFile, len + 2, "%s.o", eventName );
	
	makeFile = new char[len + 4];
	snprintf( makeFile, len + 4, "%s.znm", eventName );

	xmlFile = new char[len + 4];
	snprintf( xmlFile, len + 4, "%s.cfc", eventName );
...
int CreateZnm::createMakefile(){
	...
	if( ! makeMdfFPtr.is_open()  ){
		...
		fprintf( makeFPtr, "XML_INCLUDES:= -I /usr/local/include/QMath -I /usr/local/rtnet/include -I /usr/local/Zenom/include/ConfigDummies\nXML_LIBS:= -L /usr/local/Zenom/lib -lConfigDummies\n\n" );
	...
	fprintf( makeFPtr, "CLEAN= rm -rf $(TARGET) %s %s\n\n", makeFile, xmlFile);
	
	fprintf( makeFPtr, "# ========== Targets ==========\n" );
	
	fprintf( makeFPtr, "targets: $(TARGET) $(RUN)\n\n" );
	...
	fprintf( makeFPtr, "\t$(LD) -o %s %s $(OBJ) $(INCLUDELIBS) $(LDFLAGS)\n\t", makeFile, oFile );

	fprintf( makeFPtr, "rm %s\n\t", oFile );
		
	fprintf( makeFPtr, "$(CC) $(CFLAGS) %s $(XML_INCLUDES)\n\t", cppFile );
	
	fprintf( makeFPtr, "$(LD) -o %s %s $(XML_LIBS)\n\n", xmlFile, oFile );
