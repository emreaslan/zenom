...
Config::Config(const char* name,bool pIsConfig )
{
    ...
    isGUI = pIsConfig ;
    setFileName( name );
    mConfigFileFound =true;

    if (!mConfigXML.load_file(fileName))
    {
	mConfigFileFound = false;
...
bool Config::setFileName(const char* name)
{
    if(fileName != NULL )
        delete fileName;
    fileName = new char[LINELENGTH];
    int pathPoint = -1;

    for(int counter = 0 ; counter < LINELENGTH ; ++counter )
    {
        if( ( name[counter] == ' ' ) || ( name[counter] == '\000' ) )
            break;
        else if( name[counter] == '/' )
            pathPoint = counter;
    }

    strcpy (fileName , &(name[pathPoint+1]) ) ;
    ...
    sprintf(newFileName,"%s%s", fileName,CONFIGFILEEXTENSION);
    delete fileName;
