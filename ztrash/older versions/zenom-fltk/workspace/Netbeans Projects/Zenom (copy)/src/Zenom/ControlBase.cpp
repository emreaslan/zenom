...
ControlBase::ControlBase(int argc, char* argv[])
{
    ...
    else if( argc == 2 )
    {
        ...
        {
            copyConfigFileName ( argv[1] );
            cout << "ControlBase started in Console Mode" << endl;
        }
    }
    else if( argc == 3 )
    {
    	if (strcmp (argv[1] , CFGCOMMAND ) == 0)
        {
            copyConfigFileName ( argv[2] );
            isGUI = false;
            isConfig = true;
            cout << "ControlBase started in Configuration Mode" << endl;
        }
        else if (strcmp (argv[1] , GUICOMMAND ) == 0)
        {
            copyConfigFileName ( argv[2] );
            isGUI = true;
            isConfig = false;
        ...
        else if (strcmp (argv[2] , GUICOMMAND ) == 0 )
        {
            copyConfigFileName ( argv[1] );
            isGUI = true;
            isConfig = false;
...
bool ControlBase::loadConfigFile()
{
    ...
    config->setValue(GENERAL , "Frequency" , &frequency );
    config->setValue(GENERAL , "Duration" , &duration );
...
//==============================================================================
// ControlBase::saveConfigFile
//==============================================================================
bool ControlBase::saveConfigFile()
{
...
void ControlBase::changeConfig(){

	int err;

	err = unlink(configFileName);
	
	if( err != 0 ){
	
		perror("Old config file can mot be removed");
		return;
	
	}
	
	err = rename( config->getFileName(), configFileName );
	
	if( err != 0 )
		perror("Temp config file can not be renamed");
...
void ControlBase::run()
{
    ...
    {
        config->storeFileGUI();
        setConfigFileName((char*)"tempConfig");
        saveConfigFile();
        changeConfig();
