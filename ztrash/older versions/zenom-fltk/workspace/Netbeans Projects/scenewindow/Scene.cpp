...
Scene::~Scene()
{
    ...
    {
        delete[] vrmlFilePaths.getElement(i);
...
bool Scene::addVRML(const char* filename)
{
    ...
    if(strrchr(filename,'/'))
    {
        newVrmlFile = new char[strlen(&(strrchr(filename,'/')[1]))];
        strcpy(newVrmlFile, &(strrchr(filename,'/')[1]));
    }
    else
    {
        newVrmlFile = new char[strlen(filename)];
        strcpy(newVrmlFile, filename);
    }
    ...
    Group *vrmlRoot;
    vrmlRoot = vrmlBrowser->add_group (newVrmlFile, vrmlBrowser, NULL);
    for(int i = 0; i < newNodes->getNumElements(); ++i)
...
void Scene::removeVRML(const char* filename)
{
    for(int i = 0; i < vrmlFilePaths.getNumElements(); ++i)
    {
        if(!strcmp(filename, vrmlFilePaths.getElement(i)))
        {
            delete[] vrmlFilePaths.getElement(i);
