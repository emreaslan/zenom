...
void SceneWindow::cbAdd(Widget *o, void* userData)
{
    ...
    if(!sceneWindow->addVRMLFile(chooser.value(0)))
    	sceneWindow->enableButtons();
}
...
void SceneWindow::cbRemoveVRMLButton(Widget *o, void* userData)
{
    ...
    while(w)
    {
        if (w->selected())
        {
            ...
            for(int i = 0; sceneWindow->vrmlFiles.size(); ++i)
            {
            	if(sceneWindow->vrmlFiles[i] == w->label())
            	{
            		sceneWindow->vrmlFiles.removeElement(i);
            		break;
            	}
            }
...
int SceneWindow::addVRMLFile(const char* filename)
{
    ...
    else
    {
        ...
        vrmlFiles.appendElement(ZString(filename));
...
int SceneWindow::removeVRMLFile(const char* filename)
{
    ...
    for(int i = 0; i < bindedVariable.getNumElements(); ++i)
    {
        if(!strcmp(bindedVariable.getElement(i)->vrmlFilename.c_str(), filename))
    ...
    {
         if(bindedVariable.getElement(i)->vrmlFilename == filename)
        {
	...
	// Vrml listesinden kaldirilir.
    for(int i = 0; i < vrmlFiles.getNumElements(); ++i)
    {
    	if(vrmlFiles[i] == filename)
    	{
    		vrmlFiles.removeElement(i);
    		break;
    	}
    }
                ...
                !strcmp(wrmlVariableWidget->label(), "vector"))
        {
            for(int i = 0; i < logVariable->getRows(); ++i)
            {
                ...
                for(int j = 0; j < logVariable->getCols(); ++j)
                {
                    ...
                    newElement->vrmlFilename = vrmlFilename;
...
// SceneWindow::save
//==============================================================================
void SceneWindow::save(pugi::xml_node pSceneWindowNode)
{
    
    pSceneWindowNode.append_attribute("X") = x();
    pSceneWindowNode.append_attribute("Y") = y();
    pSceneWindowNode.append_attribute("W") = w();
    pSceneWindowNode.append_attribute("H") = h();
    pSceneWindowNode.append_attribute("Navigation_Area") = buttonGroup->opened();

    pugi::xml_node vrmlFilesNode = pSceneWindowNode.append_child("Vrml_Files");
    for (int i = 0; i < vrmlFiles.getNumElements(); ++i)
    {
        pugi::xml_node fileNode = vrmlFilesNode.append_child("File");
        fileNode.append_attribute("Path") = vrmlFiles[i].c_str();;
    }

    pugi::xml_node bindedVarsNode = pSceneWindowNode.append_child("Binded_Vars");
    for (int i = 0; i < bindedVariable.getNumElements(); ++i)
    {
        pugi::xml_node varNode = bindedVarsNode.append_child("Binded_Variable");
        varNode.append_attribute("Filename") = bindedVariable.getElement(i)->vrmlFilename.c_str();;
        varNode.append_attribute("Variable") = bindedVariable.getElement(i)->logVariable->getName();
        varNode.append_attribute("Variable_Index") = bindedVariable.getElement(i)->index;
        pugi::xml_node indexNode = varNode.append_child("Vrml_Indexes");
        for (int j = 0; j < bindedVariable.getElement(i)->vrmlIndex.getNumElements(); ++j )
            indexNode.append_attribute("Value") = bindedVariable.getElement(i)->vrmlIndex.getElement(j);

    }
}

void SceneWindow::load(pugi::xml_node pSceneWindowNode)
{
	if(pSceneWindowNode.attribute("Navigation_Area").as_bool())
		enableButtons();

    resize(pSceneWindowNode.attribute("X").as_int(),
            pSceneWindowNode.attribute("Y").as_int(),
            pSceneWindowNode.attribute("W").as_int(),
            pSceneWindowNode.attribute("H").as_int());
    layout();

    pugi::xml_node vrmlFilesNode = pSceneWindowNode.child("Vrml_Files");
    for (pugi::xml_node fileNode = vrmlFilesNode.first_child(); fileNode; fileNode = fileNode.next_sibling())
    {
    	addVRMLFile(fileNode.attribute("Path").value());
    	buttonGroup->activate();
    }
    ...
    pugi::xml_node bindedVarsNode = pSceneWindowNode.child("Binded_Vars");

    for (pugi::xml_node varNode = bindedVarsNode.first_child(); varNode; varNode = varNode.next_sibling())
    {
        // Find VRML Variable Widget
        int indexCounter = 0;
        indexes.clear();
        for (pugi::xml_attribute attNode = varNode.child("Vrml_Indexes").first_attribute(); attNode ; attNode = attNode.next_attribute() , ++indexCounter )
        {
            indexes.appendElement(attNode.as_int());
        }
        ...
        {
        	if( !strcmp(vrmlFileList->child(j)->label(), varNode.attribute("Filename").value()))
            {
                vrmlFileFound = true;
                indexes.insertElement(j,0);
                break;
            }
        }
        ...
        vrmlWidget = vrmlVariableList->goto_index(indexes.data(), indexes.size()-1);
        ...
        {
            if(!strcmp(logVariableList->child(j)->label(), varNode.attribute("Variable").value()))
            {
        ...
        if(logWidget->is_group())
        {
            ...
            logWidget = group->child(varNode.attribute("Variable_Index").as_int());
        }
        ...
        int index = 0;
        LogVariableGUI* logVariable = static_cast<LogVariableGUI*>(logWidget->user_data());
        if(logVariable->getSize() > 1)
        {
            index = logWidget->parent()->find(logWidget);
        }

        // Unbinded if vrml variable binded a log variable
        removeUnbindVariable(vrmlWidget);
        BindedVariable* newElement = new BindedVariable(vrmlWidget,logVariable, index);

        //first index vrml file name, vrml file could not load next time !
        indexes.removeElement(0);
        newElement->vrmlIndex = indexes;
        newElement->vrmlFilename = varNode.attribute("Filename").value();
        bindedVariable.appendElement(newElement);
        snprintf(buffer, 256, "%s\t%s", vrmlWidget->label() ,logWidget->label());
        vrmlWidget->copy_label(buffer);
    }

    vrmlVariableList->redraw();
}
